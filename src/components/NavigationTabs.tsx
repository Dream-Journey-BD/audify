...
import { Scissors, Sliders, Split, Send } from 'lucide-react';
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-2 sm:py-2.5">
        <div className="w-full bg-neutral-950 p-1 sm:p-1.5 rounded-xl border border-neutral-800 shadow-inner grid grid-cols-2 md:grid-cols-4 gap-1 sm:gap-2">
          <button
            id="tab-slicer-btn"
            onClick={() => onChangeTab('slicer')}
            className={`w-full inline-flex items-center justify-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-2 rounded-lg text-xs sm:text-sm font-semibold whitespace-nowrap transition-all cursor-pointer text-center ${
              activeTab === 'slicer'
          ...
          <button
            ...
            className={`w-full inline-flex items-center justify-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-2 rounded-lg text-xs sm:text-sm font-semibold whitespace-nowrap transition-all cursor-pointer text-center ${
              activeTab === 'splitter'
                ? 'bg-gradient-to-r from-emerald-500 to-teal-600 text-neutral-950 font-bold shadow-md shadow-emerald-500/20 border border-emerald-400/40'
                : 'text-neutral-400 hover:text-neutral-200'
            }`}
          >
            <Split className="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0" />
            <span className="truncate">{t.tabSplitter || 'Audio Splitter'}</span>
          </button>
